...
class ASR_Brain(sb.Brain):
    def compute_forward(self, batch, stage):
        ...
        # print("batch.sig - tensor: --->", batch.sig[0])
        # feats = [batch, time, features = 40]
        # print("feats --->", feats.shape)
        # cnn model out = [batch, time, 256]
        out = self.modules.model(feats)
        # print("out --->", out.shape)
        # rnn model out = [batch, time, 256]
        ...
        # dense layers final output -> num_class
    ...
    def on_stage_start(self, stage, epoch):
        ...
        if stage != sb.Stage.TRAIN:
            ...
            # self.frr_metrics = self.hparams.bin_stats()
    ...
    def on_stage_end(self, stage, stage_loss, epoch):
        ...
        if stage == sb.Stage.TRAIN:
            ...
            # frr = self.frr_metrics.summarize("FRR")
            # far = self.bin_metrics.summarize("FAR")
            # der = self.bin_metrics.summarize("DER")
            # precision = self.bin_metrics.summarize("precision")
            # recall = self.bin_metrics.summarize("recall")
            # Fscore = self.bin_metrics.summarize("F-score")
        ...
        if stage == sb.Stage.VALID:
            ...
            print("LR: %.4f" % old_lr)
            print("LR: %.4f" % new_lr)
            print("Train loss: %.4f" % self.train_loss)
            print(stage, "loss: %.4f" % stage_loss)
